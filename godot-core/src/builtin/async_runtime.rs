...
pub fn godot_task(future: impl Future<Output = ()> + UnwindSafe + 'static) -> TaskHandle {
...
#[cfg(feature = "trace")]
pub fn is_godot_task_paniced(task_handle: TaskHandle) -> bool {
    ASYNC_RUNTIME.with_borrow(|rt| rt.paniced_tasks.contains(&task_handle.id))
}
...
#[derive(Default)]
struct AsyncRuntime {
    tasks: Vec<FutureSlot<Pin<Box<dyn Future<Output = ()> + UnwindSafe>>>>,
    task_counter: u64,
    #[cfg(feature = "trace")]
    paniced_tasks: std::collections::HashSet<u64>,
}
...
impl AsyncRuntime {
    fn new() -> Self {
        Self {
            ...
            task_counter: 0,
            #[cfg(feature = "trace")]
            paniced_tasks: std::collections::HashSet::default(),
        }
    }
    ...
    fn add_task<F: Future<Output = ()> + UnwindSafe + 'static>(&mut self, future: F) -> TaskHandle {
        let id = self.next_id();
        let index_slot = self
            .tasks
            .iter_mut()
    ...
    fn get_task(
        ...
        index: usize,
        id: u64,
    ) -> FutureSlotState<Pin<Box<dyn Future<Output = ()> + UnwindSafe + 'static>>> {
    ...
    fn park_task(&mut self, index: usize, future: Pin<Box<dyn Future<Output = ()> + UnwindSafe>>) {
        self.tasks[index].park(future);
    }

    #[cfg(feature = "trace")]
    fn track_panic(&mut self, task_id: u64) {
        self.paniced_tasks.insert(task_id);
    }
}
    ...
    fn wake(self: std::sync::Arc<Self>) {
        let callable = Callable::from_local_fn("GodotWaker::wake", move |_args| {
            ...
            let Ok((poll_result, future)) = panic_result else {
                ASYNC_RUNTIME.with_borrow_mut(|rt| {
                    #[cfg(feature = "trace")]
                    rt.track_panic(self.task_id);
                    rt.clear_task(self.runtime_index);
                });
